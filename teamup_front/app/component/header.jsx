...
export default function Header({ user: propUser = null, profileImage: propProfileImage = null, onLogout: propOnLogout = null }) {
  ...
  useEffect(() => {
    if (propProfileImage !== undefined && propProfileImage !== null) setProfileImage(propProfileImage);
  }, [propProfileImage]);
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-3 pl-4 border-l border-gray-300">
              {/* โปรไฟล์ */}
              <div onClick={() => router.push("/profile")} className="flex items-center gap-3 cursor-pointer" title="ไปยังโปรไฟล์">
                <div className="text-right hidden sm:block">
                  <div className="text-sm font-medium text-gray-900">
                    {user && typeof user === "object" ? user.name || user.preferred_username || user.email : "ผู้เยี่ยมชม"}
                  </div>
                  <div className="text-xs text-gray-500">{user && user.email ? user.email : ""}</div>
                </div>
